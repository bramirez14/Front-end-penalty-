...
if(result.data.user){
          ...
          localStorage.setItem("name", JSON.stringify(result.data.user.nombre));
          localStorage.setItem("token", JSON.stringify(result.data.token));
          localStorage.setItem("tipo",JSON.stringify(result.data.user.tipousuario))
...
console.log(msg)
  return (
    <>
      ...
      { errores.errors===undefined ?"": errores.errors.map((err,i)=>
	 <h5 key={i}>{err.msg}</h5>)}
