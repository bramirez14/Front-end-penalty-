...
export const AnticipoSueldo = ({ history }) => {
  ...
  const { empleado, sueldo, cuotas } = anticipo;
  const [selectedDate, setSelectedDate] = useState(new Date());
  /****Funcion de Alerta******/
  const handleClickNombre = (e) => {
  ...
  const handleClickCuota = (e)=>{
    let buscarCuota= data.find(d=>e.target.value==d.id);
    setAnticipo({
      ...anticipo,
      cuotas: buscarCuota.nombre,
    });
    setOpen4(false);
  }
  ...
  <form className='formulario' onSubmit={handleSubmit}>
    ...
    <div className='item-a'>
        ...
        <InputMsg placeholder='Mensaje' width='300px' name='mensaje' change={handleChange} />
